...
from __future__ import print_function # Use a function definition from version (say 3.x from 2.7 interpreter)
...
def loadData(src):
    fname = filepath + 'cifar-10-python.tar.gz'    # if the file exists, do not download.
    print ('fname:', fname)
    if not os.path.exists(fname):
        print('Downloading ' + src)
        fname, h = urlretrieve(src, filepath +'cifar10-scrdata.me')
    print (fname, ' --Done.')
    try:
        ...
        for i in range(5):
            batchName = filepath + 'CIFAR-10/cifar-10-batches-py/data_batch_{0}'.format(i + 1)
            trn = np.vstack((trn, readBatch(batchName)))
        print('Done')
        print('Preparing test set...')
        tst = readBatch(filepath + 'CIFAR-10/cifar-10-batches-py/test_batch')
        print ('Done.')
    finally:
        pass
        # os.remove(fname)
...
# Save images
"""
In addition to saving the images in the text format, 
we would save the images in PNG format. 
In addition we also compute the mean of the image.
"""
def saveImage(fname, data, label, mapFile, regrFile, pad, **key_parms):
    # data in CIFAR-10 dataset is in CHW format.
    pixData = data.reshape((3, imgSize, imgSize))  #3*x*x
    ...
    with open('train_map.txt', 'w') as mapFile:
        with open('train_regrLabels.txt', 'w') as regrFile:
            for ifile in range(1,6):
                with open(filepath + 'CIFAR-10/cifar-10-batches-py/data_batch_' + str(ifile), 'rb') as f:
                    if sys.version_info[0] < 3:
                        data = cp.load(f)
                    else:
...
def saveTestImages(filename, foldername):
    ...
    with open('test_map.txt', 'w') as mapFile:
        with open('test_regrLabels.txt', 'w') as regrFile:
            with open(filepath + 'CIFAR-10/cifar-10-batches-py/test_batch', 'rb') as f:
                if sys.version_info[0] < 3:
                    data = cp.load(f)
                else:
                    data = cp.load(f, encoding='latin1')
